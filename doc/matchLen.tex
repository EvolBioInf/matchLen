...
\title{\texttt{matchLen} \input{version}: \emph{Description}}
\author{\input{author}}
\input{date}
...
The program \texttt{matchLen} was written in C on a computer running Linux.
Please contact \texttt{\input{email}} if there are any problems
with the program.
\begin{itemize}
\item Obtain the package\\
\texttt{git clone https://www.github.com/\input{repo}/matchLen}
\item Change into the directory just downloaded
\begin{verbatim}
cd matchLen
\end{verbatim}
and make \texttt{matchLen}
\begin{verbatim}
make
\end{verbatim}
\item Test \texttt{matchLen}
\begin{verbatim}
make test
\end{verbatim}
\item The executable \texttt{matchLen} is located in the
